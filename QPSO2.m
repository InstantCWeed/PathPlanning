function [Path Step]=QPSO2(environment,S_Position,E_Position,Parameter)
...
%第2种情况    N = 10   M=15  Iter=1000
Obst(1).x = 2;
Obst(1).y = 3;
Obst(1).tranx = cos*2 + sin*3 +Sx;
Obst(1).trany = (-sin)*2 + cos*3 +Sy;
...
Obst(4).x = 5;
Obst(4).y = 5;
Obst(4).tranx = cos*5 + sin*5 +Sx;
Obst(4).trany = (-sin)*5 + cos*5 +Sy;
...
while(i <= N)
    while(j <= M+1)
        ...
        while(k<=count)
            ...
            if(((Obst(k).tranx-(dimen/2.5))<=(j-1)*dimen <=(Obst(k).tranx + (dimen/2.5))&&(Obst(k).trany -(dimen/2.5))<= tempy <= (Obst(k).trany+(dimen/2.5))) && (((Obst(k).trany - (dimen/2.5) -Y(i,j-1))/(Obst(k).tranx  -((j-2)*dimen))) <= ((tempy - Y(i,j-1))/dimen) <= ((Obst(k).trany + (dimen/2.5) -Y(i,j-1))/(Obst(k).tranx -((j-2)*dimen)))))                
                if((-down)<Ymin)
                    tt = tt+1;
                    distance = (M+1-tt)*dimen;
                    up_var = tan*distance + (wy-Ey)/cos;
...
B = zeros(1,iter);
for k = 1:iter
    B(k) = 1- 0.99*(k/iter);%B收敛系数线性从1减为0.5
    for i = 1:N
