function [Path Step]=QPSO1(environment,S_Position,E_Position,Parameter)
...
while(i <= N)
    while(j <= M+1)
        ...
        while(k<=count)
            ...
            if(((Obst(k).tranx-(dimen/1.5))<=(j-1)*dimen <=(Obst(k).tranx + (dimen/1.5))&&(Obst(k).trany -(dimen/0.5))<= tempy <= (Obst(k).trany+(dimen/0.5))) && (((Obst(k).trany -Y(i,j-1))/(Obst(k).tranx + (dimen/1) -((j-2)*dimen))) <= ((tempy - Y(i,j-1))/dimen) <= ((Obst(k).trany -Y(i,j-1))/(Obst(k).tranx -(dimen/1) -((j-2)*dimen)))))
%             if((((j-1)*dimen -(dimen/1)) <=(Obst(k).tranx <=((j-1)*dimen +(dimen/1)))&&(tempy -(dimen/1) <= Obst(k).trany <= tempy+(dimen/1))) && (((Obst(k).trany -Y(i,j-1))/(Obst(k).tranx + (dimen/2.5) -((j-2)*dimen))) <= ((tempy - Y(i,j-1))/dimen) <= ((Obst(k).trany -Y(i,j-1))/(Obst(k).tranx -(dimen/2.5) -((j-2)*dimen)))))
...
B = zeros(1,iter);
for k = 1:iter
    B(k) = 1- 0.5*(k/iter);%B收敛系数线性从1减为0.5
    for i = 1:N
